...
message can show a clean header, live "who's answered" lines, and a
timed reveal — matching the requested look.
...
What this engine *can* match: the header format, per-answer feedback lines,
and a timed "Time's up! Answer: X" reveal with the correct option marked.
...
def _build_text(session: dict, answers: list, closed: bool, header_label: str = None) -> str:
    ...
    prefix = f"{header_label} · " if header_label else ""

    lines = [f"🧠 *Trivia* · {prefix}_{tag} ({difficulty})_", "", session["question_text"]]
...
async def send_quiz_message(chat_id: int, question: dict, points: int, context: ContextTypes.DEFAULT_TYPE,
                             kind: str = "trivia", duel_id=None, duel_round=None, header_label: str = None) -> int:
    ...
    text = _build_text(session, answers=[], closed=False, header_label=header_label)
...
async def close_quiz_session(session_id: int, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Message may have been deleted, or content unchanged — not fatal.
...
async def handle_answer_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Route scoring to the right game mode.
    ...
    # Refresh the message with the new respondent line.
    answers = db.get_quiz_answers(session_id)
    text = _build_text(session, answers, closed=False)
    markup = _build_keyboard(session, closed=False)
    try:
        await context.bot.edit_message_text(
            chat_id=session["chat_id"],
