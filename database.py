...
def init_db():
    with _connect() as conn:
        ...
        try:
            conn.execute("ALTER TABLE quiz_sessions ADD COLUMN header_label TEXT")
...
def create_quiz_session(chat_id, kind, question_text, options, correct_option,
    ...
    with _lock, _connect() as conn:
        cur = conn.execute(
            ...
            "correct_option, points, category, difficulty, duel_id, duel_round, header_label) "
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            (chat_id, kind, question_text, json.dumps(options), correct_option,
             points, category, difficulty, duel_id, duel_round, header_label),
        )
        conn.commit()
        return cur.lastrowid
...
# ==========================================================================
# Duel challenge invites (Accept/Decline before a duel actually starts)
# ==========================================================================

def create_duel_invite(chat_id, challenger_id, challenger_name, opponent_id, opponent_name):
    with _lock, _connect() as conn:
        cur = conn.execute(
            "INSERT INTO duel_invites (chat_id, challenger_id, challenger_name, "
            "opponent_id, opponent_name) VALUES (?, ?, ?, ?, ?)",
            (chat_id, challenger_id, challenger_name, opponent_id, opponent_name),
        )
        conn.commit()
        return cur.lastrowid
...
def set_invite_message_id(invite_id: int, message_id: int):
    with _lock, _connect() as conn:
        conn.execute(
            "UPDATE duel_invites SET message_id = ? WHERE invite_id = ?", (message_id, invite_id)
        )
        conn.commit()


def get_duel_invite(invite_id: int):
    with _connect() as conn:
        row = conn.execute(
            "SELECT * FROM duel_invites WHERE invite_id = ?", (invite_id,)
        ).fetchone()
        return dict(row) if row else None


def set_invite_status(invite_id: int, status: str):
    with _lock, _connect() as conn:
        conn.execute(
            "UPDATE duel_invites SET status = ? WHERE invite_id = ?", (status, invite_id)
        )
        conn.commit()


def get_pending_invite_for_chat(chat_id: int):
    with _connect() as conn:
        row = conn.execute(
            "SELECT * FROM duel_invites WHERE chat_id = ? AND status = 'pending' "
