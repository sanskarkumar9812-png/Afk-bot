...
async def send_random_question(chat_id: int, context: ContextTypes.DEFAULT_TYPE, label: str = None):
    """Sends a new question, unless one is already live in this chat — in
    which case it does nothing and returns None so the caller can decide
    how (or whether) to tell the user."""
    if db.get_open_quiz_session(chat_id, kind="trivia"):
        return None
...
async def quiz_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if db.get_open_quiz_session(chat_id, kind="trivia"):
        await update.message.reply_text(
            "A question is already live in this chat — answer it or let it run out before I send another."
        )
        return
...
async def daily_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if db.get_open_quiz_session(chat_id, kind="trivia"):
        await update.message.reply_text(
            "Hold on — there's already a question live in this chat. Finish that one first."
        )
        return

    await update.message.reply_text("Daily Challenge — answer correctly to extend your streak.")
    try:
        await send_random_question(chat_id, context, label="Daily")
    except TelegramError:
...
async def _autoquiz_job_callback(context: ContextTypes.DEFAULT_TYPE):
    try:
        session_id = await send_random_question(context.job.chat_id, context)
        if session_id is None:
            logger.info("Autoquiz skipped in %s — a question was already live.", context.job.chat_id)
